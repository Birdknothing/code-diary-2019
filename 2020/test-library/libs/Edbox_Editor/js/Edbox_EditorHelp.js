...
(function (namespace, className) {
     ...
     * MD5组件路径
     */
    var MD5Path = Edbox.ComponentRootPath + "ThirdParty/md5/md5.min.js";

    /**
     * 帮助手册域名
     */
    var Host = {
        CN: "help.edbox-cn.101.com",
        US: "help.edbox.101.com"
    };
    ...
    GetHelpInfo = function (baseId, baseVersion, lang, success, error) {
        ...
        function GetHost(){
            if (Edbox.ServerKey === "US" || "Beta" === Edbox.ServerKey){
                return Host.US;
            }
            else{
                return Host.CN;
        ...
        subUrl += "?playerid=" + Edbox.EbUserId + "&productid=" + baseId + "&version=" + baseVersion;
        ...
        Edbox.Request.Get(Edbox.GetHost("MMO"), subUrl, null, function (data) {
            ...
            Require([MD5Path], function () {
                ...
                var url = Edbox.Protocol + "://" + GetHost() + HelpConfig.Url + param;
    ...
    /**
     * Edbox编辑器帮助手册组件
     * 用于Edbox平台的编辑器帮助手册服务框架
     * @author 陈五洲(880123)
     * @see http://ndsdn.nd.com.cn/index.php?title=Edbox帮助手册组件
     * */
    var module = {
        ...
        * @param {Function} success 成功回调,  { bool } isopen 是否显示
        ...
        Init: function (success, error) {
            ...
            var language = getLang(Edbox.Language);
            ...
            if (Edbox.Access === 1){
                GetHelpInfo(Edbox.GameId, Edbox.Version, language, success, error);
            }
            else if (Edbox.Access === 2){
                Edbox.Api.MMO.GetPersonalAppInfo(Edbox.GameId, function(tempInfo){
