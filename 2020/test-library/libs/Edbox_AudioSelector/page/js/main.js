var Manager = {
    ...
    PostToNDR: function (info) {
        function NameCheck(info, success, error) {
            ...
            var extend = info.Name.substring(index).toLowerCase();
            if(extend !== '.wav' && extend !== '.ogg' && extend !== '.wma' && extend !== '.mp3' && extend !== '.m4a'){
                if (error) error("Non audio file");
                return;
            }
        ...
        NameCheck(info, function () {
            ...
            Edbox.NDR.Post(info.Data, info.Name, func.Progress, func.Success, func.Error);
        }, func.Error);
    },
    ...
    Online: {
        ...
        SelectFirstTab: function (index) {
            ...
            Edbox.FrontendLib.GetSortTree(tag, function (data) {
        ...
        Search: function (append) {
            ...
            Edbox.FrontendLib.GetResources(tag, key, page, size, function (data) {
                ...
                function handle(key, value) {
                    Edbox.NDR.Get(value.id, function (data) {
        ...
        Init: function () {
            ...
            if (DataFunction.Online.Formats.length === 0 && !Edbox.Platform.IsPC) {
