    ...
    var module = {
        ...
        UploadImageToCS: function (file, success, error, process) {
            Edbox.NDR.GetFileData(file, function (info) {
                Edbox.Action.FeedBack.NameCheck(info, function () {
                    var userid = Edbox.EbUserId;
                    Edbox.Api.NDR.GetSession(userid, function (session) {
                        function ReturnCsData(csdata) {
                            var data = new Object();
                            if (csdata) {
                                data.dentry_id = csdata.dentry_id;
                                data.url = Edbox.Protocol + "://" + Edbox.GetHost("CDNCS") + "/v0.1/static" + csdata.path;
                                success(data);
                            }
                            else {
                                var ex = new Object();
                        ...
                        var blob = Edbox.NDR.Base64ToBlob(info.Data);
                        Edbox.Api.CS.UpLoadToCS(session.session_id, session.dist_path, info.Name, blob, process, ReturnCsData, error);
                    }, error);
                }, error);
            });
        },
        ...
        */
        ...
        GetCSImageUrl: function (dentry_id) {
            ...
            data.url = Edbox.Protocol + "://" + Edbox.GetHost("CDNCS") + "/v0.1/download?dentryId=" + dentry_id;
            return data;
        },
            ...
            var succ = function (dataObj) {
                ...
                if (!dataObj.hasOwnProperty('items')) {
                    ...
                    if (dataObj.items[0]) {
                        ...
                        if (modelName === "Edbox_Lobby") {
                            Edbox.Action.FeedBack._lobbyModel = modelData;
                        } else {
                            Edbox.Action.FeedBack._editorModel = modelData;
                        }
                        success(modelData);
                    } else {
                        var err1 = new Object();
       ...
       */
        PostFeedBack: function (modelName, contact, feedback_type, content, scene, subscript, screenshot, success, error) {
            var model_id = "";
            if (modelName === "Edbox_Lobby") {
                model_id = Edbox.Action.FeedBack._lobbyModel.model_id;
            }
            else {
                model_id = Edbox.Action.FeedBack._editorModel.model_id;
            }
