...
(function (namespace, className) {
    ...
    function Get(guid, success, error) {
        api.Get(guid, function (data) {
            var url = data.tech_info.source.location.replace('${ref-path}', Edbox.Protocol + "://" + Edbox.GetHost("CDNCS") + "/v0.1/static");
    ...
    function GetPreview(guid, success, error) {
        api.Get(guid, function (data) {
            ...
            var url = data.preview.png.replace('${ref-path}', Edbox.Protocol + "://" + Edbox.GetHost("CDNCS") + "/v0.1/static");
    ...
    function GetAtlas(guid, success, error) {
        api.Get(guid, function (data) {
            ...
            var url = data.tech_info.atlas.location.replace('${ref-path}', Edbox.Protocol + "://" + Edbox.GetHost("CDNCS") + "/v0.1/static");
            if (success) success(url);
        }, error);
    }
    ...
    function Set(data, preview, atlas, progress, success, error) {
        ...
        api.GetSession(userid, function (sessionData) {
            ...
            function UploadPreview() {
                if (preview) {
                    var blob = Edbox.NDR.Base64ToBlob(preview);
            ...
            function UploadAtlas() {
                if (atlas) {
                    var blob = Edbox.NDR.Base64ToBlob(atlas);
            ...
            function UploadZip() {
                GetZip(data, function (blob) {
                    ...
                    Edbox.Api.CS.UpLoadToCS(sessionData.session_id, sessionData.dist_path, "data.gzip", blob, function (per, loader, total) {
    ...
    var module = {
         ...
         * @param {String} error 失败回调
         */
        GetPreview: function (guid, success, error) {
         ...
         * @param {String} error 失败回调
         */
        GetAtlas: function (guid, success, error) {
         ...
         * @param {Function} error 失败回调
         */
        Set: function (data, preview, atlas, progress, success, error) {
