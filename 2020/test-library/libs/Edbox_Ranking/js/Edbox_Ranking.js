 ...
 * @see http://ndsdn.nd.com.cn/index.php?title=Edbox%E9%80%9A%E7%94%A8%E6%8E%92%E8%A1%8C%E6%A6%9C%E7%BB%84%E4%BB%B6Html5%E7%89%88
 * Js需求:
 *      jquery
 *      CryptoJS
 *      Edbox
...
Edbox.Ranking = {
     ...
     */
    Show: function () {
        var url = Edbox.Ranking.ComponentPath + "page/";
        ...
        function hide(data) {
            if (data.data === "Edbox.Ranking.Hide") {
     ...
     */
    Hide: function () {
        window.postMessage("Edbox.Ranking.Hide", '*');
    },
    ...
    Get: function (page, size, success, error) {
        var data = "id=" + Edbox.Ranking.ID + "&player_id=" + Edbox.EbUserId + "&page=" + page + "&size=" + size;
    ...
    Post: function (score, success, error) {
        var data = new Object();
        data.ids = Edbox.Ranking.ID;
        data.app_id = Edbox.GameId;
        data.player_id = Edbox.EbUserId - 0;
        data.player_name = Edbox.UserName;
        data.player_avatar = Edbox.Avatar;
