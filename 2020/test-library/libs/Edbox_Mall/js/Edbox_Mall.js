 ...
 */
Edbox.Mall.prototype.Get = function (success, error) {
    var self = this;
    var subUrl = "/v0.1/api/mall/mall/actions/get_mall_info_by_guid";
    var data = "guid=" + self.GUID;
    Edbox.Mall.ServerGet(subUrl, data, function (data) {
        self.GUID = data.guid;
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Mall.prototype.Set = function (success, error) {
    var self = this;
    ...
    var obj = {
        ...
        res_id: Edbox.PackageGuid,
        person_guid: Edbox.GameId,
        person_version: Edbox.Version
    };
    Edbox.Mall.ServerPost(subUrl, obj, function (data) {
        if (data.guid)
            self.GUID = data.guid;
        if (success) success(self);
    }, error);
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Mall.prototype.Delete = function (success, error) {
    var self = this;
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Mall.prototype.CreateStore = function (success, error) {
    var self = this;
    ...
    Edbox.Mall.ServerPost(subUrl, obj, function (data) {
        var store = new Edbox.Store({ GUID: data.guid });
        store.ID = store && store.ID || Edbox.GetGUID();

        store.StoreName = obj.name;
        store.Mall = self;
        Edbox.Array.Add(self.StoreList, store);
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Mall.prototype.Open = function (success, error) {
    var com = Edbox.Mall;
    ...
    var data = new Object();
    var json = Edbox.Mall.GetJson();
    data.Datas = JSON.parse(json).Mall;

    data.Page = "Mall";

    json = JSON.stringify(Edbox.Economic.CurrencyList);
 ...
 * Default Value : null
 */
Edbox.Mall.CanBuyJudgeFunction = null;
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Mall.Init = function (data, success, error) {
    Edbox.Mall.CanBuyJudgeFunction = function (commodity, count) {
        return true;
    };
    ...
    var info = new Edbox.Mall(obj);
    info.ID = info && info.ID || Edbox.GetGUID();
    ...
    function getGameMallGUID(success, error) {
        ...
        var data = "person_guid=" + Edbox.GameId + "&person_version=" + Edbox.Version;
        Edbox.Mall.ServerGet(subUrl, data, function (data) {
            info.GUID = data.guid;
    ...
    function HandleMallData(ans, com, obj) {
        ...
        for (var i = 0; i < obj.StoreList.length; i++) {
            ...
            var info = new Edbox.Store(store);
            console.log(store);
            info.Get(function (data) {
    ...
    function HandleCommodityData(ans, com, obj) {
        Edbox.Array.Add(com.CommodityList, ans);
        var info = new Edbox.Prop(obj.BindingProp);
        info.Get(function (data) {
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Mall.GetList = function (success, error) {
    if (!Edbox.Mall.Current || !Edbox.Mall.Current.StoreList) {
        if (error) error("Uninitialized");
        return;
    }
    if (success) success(Edbox.Mall.Current.StoreList);
};
 ...
 */
Edbox.Mall.GetJson = function () {
    var current = Edbox.Mall.Current;
    var conf = new Object();
    ...
    function HandleCommodityList(current, conf) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            Edbox.Array.Add(conf.CommodityList, info);
        }
    }
    ...
    function HandleStoreList(current, conf) {
        ...
        for (i = 0; i < list.length; i++) {
            ...
            Edbox.Array.Add(conf.StoreList, info);
        }
    }
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Mall.OpenMall = function (success, error) {
    Edbox.Mall.Current.Open(success, error);
};
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Store.prototype.Get = function (success, error) {
    var self = this;
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Store.prototype.Set = function (success, error) {
    var self = this;
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Store.prototype.Delete = function (success, error) {
    var self = this;
    ...
    Edbox.Mall.ServerPost(subUrl, obj, function (data) {
        Edbox.Array.Remove(self.Mall.StoreList, self);
        if (success) success();
    }, error);
};
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Store.prototype.CreateCommodity = function (success, error) {
    var self = this;

    var commodity = new Edbox.Commodity({
        ID: Edbox.GetGUID(),
    ...
    commodity.Set(function () {
        Edbox.Array.Add(self.CommodityList, commodity);
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Store.prototype.Open = function (success, error) {
    var self = this;
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Store.GetByGUID = function (guid, success, error) {
    Edbox.Mall.GetList(function (list) {
        for (var i = 0; i < list.length; i++) {
            if (list[i].GUID === guid) {
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Commodity.prototype.Get = function (success, error) {
    var self = this;
    ...
    Edbox.Mall.ServerPost(subUrl, obj, function (data) {
        if (data && data.items && data.items.length > 0) {
            ...
            self.StartSaleTime = Edbox.Mall.GetTime(data.items[0].start_time_sell);
            self.EndSaleTime = Edbox.Mall.GetTime(data.items[0].end_time_sell);

            self.Price = Edbox.Mall.ParsePriceArray(data.items[0].item_payment_list);
        }
        if (success) success(self);
    }, error);
};
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Commodity.prototype.Set = function (success, error) {
    var self = this;
    ...
    var obj = {
        ...
        // 限购时间
        start_time_sell: self.StartSaleTime ? Edbox.Mall.GetTimeNumber(self.StartSaleTime) : 0,
        end_time_sell: self.EndSaleTime ? Edbox.Mall.GetTimeNumber(self.EndSaleTime) : 0,

        payment_arr: Edbox.Mall.GetPriceArray(self.Price)
    };
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Commodity.prototype.Delete = function (success, error) {
    var self = this;
    ...
    Edbox.Mall.ServerPost(subUrl, obj, function (data) {
        Edbox.Array.Remove(self.Store.CommodityList, self);
        if (success) success();
    }, error);
};
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Commodity.prototype.Buy = function (Currency, Count, success, error) {
    var self = this;
    ...
    if (price) {
        obj.pay_item_id = Edbox.Currency.GetGoldType(Currency.Index);
    ...
    if (!Edbox.Mall.CanBuyJudgeFunction(self, Count)) {
    ...
    Edbox.Mall.ServerPost(subUrl, obj, function (data) {
        if (Edbox.Backpack) {
            Edbox.Backpack.GainPropByMall(self.BindingProp, self.BindingPropCount * Count);
        }
        Edbox.Economic.UseCurrencyByMall(Currency, Count);
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Commodity.prototype.Open = function (success, error) {
    var self = this;
 ...
 * @param {Function} error 出错回调 , 带Function参数Object , 允许为空
 */
Edbox.Commodity.GetByGUID = function (guid, success, error) {
    Edbox.Mall.GetList(function (list) {
        for (var i = 0; i < list.length; i++) {
...
// 获取时间数字
Edbox.Mall.GetTimeNumber = function (date) {
...
// 时间数字转时间
Edbox.Mall.GetTime = function (num) {
...
// 获取价格数组
Edbox.Mall.GetPriceArray = function (prices) {
    var array = new Array();
    ...
    for (var i = 0; i < keys.length; i++) {
        var key = keys[i];
        var obj = {
            pay_item_id: Edbox.Currency.GetGoldType(key.Index),
            money_type: 0,
            price: prices.Get(key)
        };
        Edbox.Array.Add(array, obj);
    }
    return array;
};

// 价格数组转化
Edbox.Mall.ParsePriceArray = function (arr) {
    var map = new Dictionary();
    for (var i = 0; i < arr.length; i++) {
        map.Set(Edbox.Currency.GetByGUID(arr[i].payItemId), arr.price);
    }
    return map;
};

// Get请求方式
Edbox.Mall.ServerGet = function (subUrl, data, success, error) {
    Edbox.Request.GetWithEnv(Edbox.GetHost("MMO"), subUrl, data, success, error);
};

// Post请求方式
Edbox.Mall.ServerPost = function (subUrl, obj, success, error) {
    Edbox.Request.PostWithEnv(Edbox.GetHost("MMO"), subUrl, JSON.stringify(obj), success, error);
};

// 初始化组件路径
Edbox.InitPath(Edbox.Mall);

// 初始化需求组件
Edbox.InitRequire(Edbox.ComponentRootPath + "Edbox_Prop/js/Edbox_Prop.js", "undefined" === typeof Edbox.Prop);
Edbox.InitRequire(Edbox.ComponentRootPath + "Edbox_Economic/js/Edbox_Economic.js", "undefined" === typeof Edbox.Economic);

// 初始化UI绘制
Edbox.Mall.InitFrame = function () {
    var com = Edbox.Mall;
    ...
    var Functions = {
        ...
        Buy: function (datas) {
            ...
            var count = datas[2];
            if (!guid) return;
            Edbox.Commodity.GetByGUID(guid, function (data) {
                Edbox.Currency.GetByGUID(currencyguid, function (currency) {
