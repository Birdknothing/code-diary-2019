...
const path = require("path"),
    ...
    scssRules = [
        { test: /\.scss$/, sideEffects: true, use: ["style-loader", "css-loader", { loader: "sass-loader", options: { outputStyle: "compressed" } }] }
    ],
    cssPlugins = [],
    ...
    fs.readdirSync(dir).forEach(furl => {
        ...
        if (stat.isFile()) {
            ...
            if (fname === "index" && entryExt.includes(extname)) {
                ...
                scssRules.push({
                    test: new RegExp(`${htmlName}.scss`),
                    sideEffects: true,
                    use: cssPlugins[cssPlugins.length - 1].extract(["css-loader", { loader: "sass-loader", options: { outputStyle: "compressed" } }])
                });
            }
        }
    });
