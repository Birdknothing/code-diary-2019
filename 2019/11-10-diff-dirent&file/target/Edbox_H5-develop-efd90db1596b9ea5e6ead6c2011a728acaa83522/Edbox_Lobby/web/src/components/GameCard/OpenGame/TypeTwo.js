...
class TypeTwo extends Component {
    ...
    handleOpenGame = id =>{
        ...
        Edbox.DataStatistic.ClickEvent('TrialGame',statisticCategory,title)
    }
    ...
    render() {
        ...
        return (
            <div className={`${styles['btn-option02']} ${isLoading ? styles['loading'] : ''}`}>
                <span onClick={()=>this.handleOpenGame(id)} className={`${styles['play02']} ${isLoading ? styles['loading'] : ''}`}>{isLoading? <Icon type="loading" style={{ fontSize: 12,position:'absolute',top:'50%',left:'5px',marginTop:'-5px' }} spin /> : ''}{isLoading ? 'Loading' : formatMessage({id:'trail'})}</span>
