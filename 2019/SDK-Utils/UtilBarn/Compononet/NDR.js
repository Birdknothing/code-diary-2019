...
(function (namespace, className) {
    ...
    function getName(data) {
        ...
        if (UtilBarn.Language === "English" && data.global_title["en-US"]) return data.global_title["en-US"];
        if (UtilBarn.Language === "TraditionalChinese" && data.global_title["zh-HK"]) return data.global_title["zh-HK"];
        if (UtilBarn.Language === "TraditionalChinese_TW" && data.global_title["zh-TW"]) return data.global_title["zh-TW"];
        if (UtilBarn.Language === "SimplifiedChinese" && data.global_title["zh-CN"]) return data.global_title["zh-CN"];
        return ans;
    }

    /**
     * UtilBarn NDR组件
     * 用于UtilBarn平台的访问NDR服务器上传下载过程
     * @author 温荣泉(201901)
     * @see http://ndsdn.nd.com.cn/index.php?title=UtilBarnNDR%E7%BB%84%E4%BB%B6JS%E7%89%88
     */
    var module = {
        /**
         * 请求资源
        ...
        Get: function (guid, success, error, formats) {
            ...
            api.Get(guid, function (ans) {
                ...
                if (ans.preview && ans.preview.png) {
                    obj.Preview = ans.preview.png.replace('${ref-path}', UtilBarn.Protocol + "://" + UtilBarn.GetHost("CDNCS") + "/v0.1/static");
                }
                obj.Url = ans.tech_info.source.location.replace('${ref-path}', UtilBarn.Protocol + "://" + UtilBarn.GetHost("CDNCS") + "/v0.1/static");
                ...
                for (var i = 0; i < keys.length; i++) {
                    ...
                    obj.Url = info.location.replace('${ref-path}', UtilBarn.Protocol + "://" + UtilBarn.GetHost("CDNCS") + "/v0.1/static");
        ...
        Post: function (data, name, progress, success, error) {
            ...
            name = getFileName(name) + "_" + UtilBarn.GetGUID() + '.' + getExtend(name);
            ...
            function uploadToCS(sessionData) {
                ...
                session = sessionData;
                uploadAjax = sendAjax = UtilBarn.Api.CS.UpLoadToCS(sessionData.session_id, sessionData.dist_path, name, blob, function (per, loader, total) {
