...
(function (namespace, className) {
    ...
    /**
     * UtilBarn MMO组件
     * 用于UtilBarn平台的访问MMO服务器游戏发布试玩过程
     * @author 温荣泉(201901)
     * @see http://ndsdn.nd.com.cn/index.php?title=UtilBarnMMO%E7%BB%84%E4%BB%B6JS%E7%89%88
     * */
    var module = {
        /**
         * 获取模板或个人作品信息
         * 集成方法,返回效率受UtilBarn.Access的设置影响
         * @param {String} productid 模板id
         * @param {Function} success 成功回调,带Object类型参数,access_type定义类型 1、模板库 2、个人作品
         * @param {Function} error 出错回调
         * @param {Boolean} change 是否调整UtilBarn.Access，默认不调整
         */
        GetInfo: function (productid, success, error, change) {
            ...
            // 假设UtilBarn.Access与请求的productid的类型一致
            // 不一致时溢出使用其他方法请求
            // 若其他方法成功返回，且change为true将修改UtilBarn.Access
        ...
        */
        ...
        CreateTempProduct: function (productid, version, packageid, productname, icon, introduction, success, error, infodata) {
            ...
            function createByInfodata() {
                ...
                if (introduction.length > 1000) {
                    alert(UtilBarn.GetTip("ERROR_LengthExceeds"));
                    if (error)
                        error(UtilBarn.GetTip("ERROR_LengthExceeds"));
                    return;
                }
                ...
                module.IsNameDuplicate(productid, 1, productname, 2, function (ans) {
                    if (ans.is_duplicate === 1) {
                        if (error)
                            error(UtilBarn.GetTip("ERROR_NameDuplicate"));
                        return;
                    }
                    create(infodata);
                }, error);
        ...
        CopyProduct: function (productid, productname, version, success, error) {
            api.CopyPersonalApp(productid, version, productname, function (data) {
                ...
                if (data.responseJSON.code === "UtilBarn_LOBBY/PRODUCT_NAME_DUPLICATE") {
        ...
        GetGameInfo: function (productid, version, access, mode, success, error) {
            if (!version) version = "";
            api.GetTryPlayingInfo(productid, access, mode, version, function (data) {
                if (!data.web_resid) {
