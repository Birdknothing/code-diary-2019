    ...
    var module = {
		...
		SearchBaiduImage: function (data, success, error) {
			module.BaseServiceCheck("SEARCHERROR", function(){
				...
				UtilBarn.WebSocket.SearchBaiduImageResultCbDic.Set(taskId, function (returnTaskId, data) {
			...
			module.BaseServiceCheck("SEARCHERROR", function(){
				...
				var config = {
					...
					success: function (ans) {
						...
						UtilBarn.WebSocket.Send(message);
						UtilBarn.WebSocket.RequestBase64ImageCbDic.Set(taskId, function (returnTaskId, data) {
							if (returnTaskId === taskId) {
								if (data.Code === "SUCCESS") {
									var result = {};
									result.Code = data.Code;
		...
		OpenScreenShooter: function (data, success, error) {
			module.BaseServiceCheck("SCREENSHOOTER", function(){
				...
				UtilBarn.WebSocket.Send(message);
				UtilBarn.WebSocket.OpenScreenShooterCbDic.Set(taskId, function (returnTaskId, data) {
					if (returnTaskId === taskId) {
						if (data.Code === "SUCCESS") {
							var result = {};
							result.Code = data.Code;
